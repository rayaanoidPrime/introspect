    ...
    <div className="w-3/4 p-6 border border-gray-200 rounded-lg shadow-lg">
      ...
      >
          ...
          <div key={index} className="flex space-x-4 mb-4">
            ...
            <div className="w-1/4">
              ...
              <select
                className="w-full border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm p-2"
                value={user.userType}
                onChange={(e) => handleChange(index, "userType", e.target.value)}
                required
                disabled={loading}
              >
            ...
            <div className="w-1/4">
              ...
              <select
                ...
                className="w-full border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm p-2"
                value={user.userDb}
                onChange={(e) => handleChange(index, "userDb", Array.from(e.target.selectedOptions, option => option.value))}
                disabled={loading}
              >
                {allowedDbs.map((db) => (
        ...
        <button
          ...
          disabled={loading || csvString.trim() === "username,password,user_type,allowed_dbs"}
        >
          Add Users
        </button>
      </form>
